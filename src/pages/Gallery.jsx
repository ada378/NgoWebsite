    ...
    <div>
      <section className="relative bg-black text-white py-32">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <Camera className="w-16 h-16 text-secondary mx-auto mb-6" />
          <h1 className="text-5xl md:text-6xl font-heading font-bold mb-4">Our Gallery</h1>
          <p className="text-xl text-gray-400 max-w-2xl mx-auto">Capturing moments of impact, transformation, and hope from our programs across India.</p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {filteredItems.map((item, index) => (
              <div key={item.id} onClick={() => { setCurrentIndex(index); setLightboxOpen(true) }}
                className="relative group cursor-pointer overflow-hidden rounded-xl aspect-square border border-gray-800 hover:border-secondary/50 transition-colors">
                ...
                <div className="absolute top-4 right-4 bg-secondary/90 rounded-full p-2 opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      {lightboxOpen && (
        <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center">
          <button onClick={() => setLightboxOpen(false)} className="absolute top-6 right-6 text-white hover:text-gray-300 transition-colors">
            <X className="w-8 h-8" />
          </button>
          <button onClick={() => setCurrentIndex((prev) => (prev - 1 + filteredItems.length) % filteredItems.length)} className="absolute left-6 text-white hover:text-gray-300 transition-colors">
            <ChevronLeft className="w-12 h-12" />
          </button>
          <button onClick={() => setCurrentIndex((prev) => (prev + 1) % filteredItems.length)} className="absolute right-6 text-white hover:text-gray-300 transition-colors">
            <ChevronRight className="w-12 h-12" />
          </button>
          <div className="max-w-5xl max-h-[80vh] mx-4">
            <img src={filteredItems[currentIndex].src.replace('w=600&h=400', 'w=1200&h=800')}
              alt={filteredItems[currentIndex].title} className="max-w-full max-h-[70vh] object-contain rounded-lg" />
            <div className="text-white text-center mt-6">
              <p className="text-2xl font-bold">{filteredItems[currentIndex].title}</p>
              <p className="text-gray-400">{filteredItems[currentIndex].location}</p>
              <p className="text-sm text-gray-500 mt-2">{currentIndex + 1} / {filteredItems.length}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <section className="py-20 bg-black">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl font-heading font-bold mb-6 text-white">Want to See More?</h2>
          <p className="text-xl text-gray-400 mb-8">Follow us on social media for regular updates and more photos from our programs.</p>
          <div className="flex justify-center space-x-4">
            {['Facebook', 'Instagram', 'YouTube'].map((social) => (
              <a key={social} href="#" className="px-6 py-3 bg-gray-900 text-gray-300 rounded-lg hover:bg-gray-800 border border-gray-800 transition-colors">
                {social}
              </a>
            ))}
          </div>
